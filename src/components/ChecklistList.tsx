  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-slide-in">
      <div className="checklist-undo-toast flex items-center gap-3 px-5 py-3 rounded-xl shadow-lg border"
        ...
        <button onClick={onUndo} className="px-3 py-1 text-xs font-bold text-primary hover:bg-primary/10 rounded-lg">{t('checklists.expandedItems.undo')}</button>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          {checklist.endDate && (
            ...
            {t('checklists.priorityLabel', { priority })}
          </span>
        </div>
        {effort > 0 && (
          <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>
            {t('checklists.estEffort', { hours: Math.floor(effort / 60), minutes: effort % 60 })}
          </span>
        )}
      </div>
...
function InlineItemInput({ onAdd, checklistItems }: { onAdd: (title: string, priority: Priority, parentId?: number | null) => void; checklistItems: ChecklistItem[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex gap-2 mt-3">
      <input ref={inputRef} type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder={t('checklists.expandedItems.addItem')}
    ...
    <div className="px-4 pb-4 border-t pt-3" style={{ borderColor: 'var(--color-border)' }}>
      {completedCount > 0 && mode !== 'history' && (
        <div className="flex items-center justify-end mb-2">
          <button onClick={() => setHideCompleted(!hideCompleted)}
            ...
            {hideCompleted ? t('checklists.expandedItems.showCompleted', { count: completedCount }) : t('checklists.expandedItems.hideCompleted')}
          </button>
        </div>
      )}
      ...
      {visibleItems.length === 0 ? (
        <p className="text-sm text-center py-3" style={{ color: 'var(--color-text-muted)' }}>
          {hideCompleted ? t('checklists.expandedItems.allCompleted') : t('checklists.expandedItems.noItems')}
        </p>
      ) : (
              ...
              <div key={item.id}>
                ...
                {notesId === item.id && mode !== 'history' && (
                  <div className="ml-10 mt-1 mb-2" style={{ marginLeft: `${depth * 20 + 40}px` }}>
                    <textarea value={notesValue} onChange={e => setNotesValue(e.target.value)} rows={2} placeholder={t('checklists.expandedItems.notesPlaceholder')}
                    ...
                    <div className="flex gap-2 mt-1">
                      <button onClick={() => { onUpdateItem(c.id, item, { notes: notesValue || null }); setNotesId(null) }}
                        className="btn-neon px-3 py-1 text-xs bg-primary text-white rounded-lg hover:bg-primary/90">{t('common.save')}</button>
                      <button onClick={() => setNotesId(null)}
                        className="btn-outline px-3 py-1 text-xs checklist-cancel-btn" style={{ color: 'var(--color-text-muted)' }}>{t('common.cancel')}</button>
                    </div>
                  </div>
                )}
              </div>
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="empty-state text-center py-16 rounded-xl border border-dashed"
          ...
          <p className="mt-1" style={{ color: 'var(--color-text-muted)' }}>{search ? t('checklists.tryDifferentSearch') : tab === 'templates' ? t('checklists.createTemplateHint') : tab === 'history' ? t('checklists.archiveHint') : t('checklists.createHint')}</p>
        </div>
      ) : (
                ...
                <div className="p-4 cursor-pointer group" onClick={() => onToggleExpand(c.id)}>
                  ...
                  <div className="mt-3 flex items-center gap-3 ml-6">
                    ...
                    <span className="text-xs font-medium w-16 text-right" style={{ color: 'var(--color-text-muted)' }}>
                       {(() => {
                         const itemsList = Array.isArray(c.items) ? c.items : []
                         return t('checklists.itemsCount', { completed: itemsList.filter(i => i.completed).length, total: itemsList.length, progress })
                       })()}
                    </span>
                  </div>
                  {/* History metrics */}
                  {isHistory && (c as any).metrics && (
                    <div className="mt-2 ml-6 flex gap-3 text-xs" style={{ color: 'var(--color-text-muted)' }}>
                      <span>{t('checklists.historyMetrics.items', { count: (c as any).metrics.totalItems })}</span>
                      <span>{t('checklists.historyMetrics.completed', { count: (c as any).metrics.completedItems })}</span>
                      <span>{t('checklists.historyMetrics.completion', { count: (c as any).metrics.completionPercentage })}</span>
                      {(c as any).metrics.totalEffort > 0 && <span>{t('checklists.historyMetrics.effort', { hours: Math.floor((c as any).metrics.totalEffort / 60), minutes: (c as any).metrics.totalEffort % 60 })}</span>}
                    </div>
                  )}
                </div>
      ...
      {undoChecklist && <UndoToast message={t('checklists.expandedItems.deleted', { title: undoChecklist.checklist.title })} onUndo={handleUndo} onClose={dismissUndo} />}
    </div>
  )
}
